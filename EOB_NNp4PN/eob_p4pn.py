...
class EOB(eqx.Module):
    ...
    radiative_order: str
    A_p4pn: eqx.nn.Sequential #NN for 4+ PN contributions to Schwarzschild potential A
    D_p4pn: eqx.nn.Sequential #NN for 4+ PN contributions to Schwarzschild potential D
    F_p_4_nqc: eqx.nn.Sequential #NN for 4+ PN contributions to GW flux F
    ...
    def __init__(
        ...
        for _ in range(num_layers_a):
            key , key_layer = jax.random.split(key,2)
            A_net.append(eqx.nn.Linear(current_input_dim, hidden_dim_a, key=key_layer))
            A_net.append(jax.nn.tanh)
            current_input_dim = hidden_dim_a
        A_net.append(eqx.nn.Linear(current_input_dim, "scalar", key=key))
        self.A_p4pn = eqx.nn.Sequential(A_net)
        
        current_input_dim = 2
        D_net = []
        for _ in range(num_layers_d):
            key , key_layer = jax.random.split(key,2)
            D_net.append(eqx.nn.Linear(current_input_dim, hidden_dim_d, key=key_layer))
            D_net.append(jax.nn.tanh)
            current_input_dim = hidden_dim_d
        D_net.append(eqx.nn.Linear(current_input_dim, "scalar", key=key))
        self.D_p4pn = eqx.nn.Sequential(D_net)
        
        current_input_dim = 3
        F_net = []
        for _ in range(num_layers_f):
            key , key_layer = jax.random.split(key,2)
            F_net.append(eqx.nn.Linear(current_input_dim, hidden_dim_f, key=key_layer))
            F_net.append(jax.nn.tanh)
            current_input_dim = hidden_dim_f
        F_net.append(eqx.nn.Linear(current_input_dim, "scalar", key=key))
        self.F_p_4_nqc = eqx.nn.Sequential(F_net)
    ...
    def _a_potential(self, r, constants):
        ...
        """
        u = 1 / r
        a = self.pade_1_4(u, constants['a_1'], constants['a_3'], constants['a_4'])
        return a
