...
void SEOBNRv5_aligned_spin_NQC_corrections(commondata_struct *restrict commondata) {
  ...
  for (i = 0; i < commondata->nsteps_fine; i++) {
    ...
    commondata->waveform_inspiral[IDX_WF(i + commondata->nsteps_low, STRAIN)] =
        nqc_amp * commondata->waveform_fine[IDX_WF(i, STRAIN)] * cexp(I * nqc_phase);
  }
} // END FUNCTION SEOBNRv5_aligned_spin_NQC_corrections
