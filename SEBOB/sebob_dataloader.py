...
"""
    ...
    def _single_trajectory_load(self,x):
        ...
        commondata = self._single_commondata_load(x)
        nu = x[0]
        dynamics_fine = commondata.dynamics_fine
        times = np.array([dynamics_fine[idx(i,TIME)] for i in range(commondata.nsteps_fine)])
        new_times = np.linspace(times[0],times[-1],self.srate)
        r_spl = CubicSpline(times,np.array([dynamics_fine[idx(i,R)] for i in range(commondata.nsteps_fine)]))
        phi_spl = CubicSpline(times,np.array([dynamics_fine[idx(i,PHI)] for i in range(commondata.nsteps_fine)]))
        prstar_spl = CubicSpline(times,np.array([dynamics_fine[idx(i,PRSTAR)] for i in range(commondata.nsteps_fine)]))
        pphi_spl = CubicSpline(times,np.array([dynamics_fine[idx(i,PPHI)] for i in range(commondata.nsteps_fine)]))
        r = r_spl(new_times)
        phi = phi_spl(new_times)
        prstar = prstar_spl(new_times)
        pphi = pphi_spl(new_times)
        rdot = r_spl(new_times,1)
        phidot = pphi_spl(new_times,1)
...
if __name__ == "__main__":
    ...
    if not trajectory:
        ...
        x, y = seob_loader(42, 160, 0.01, 0.03, trajectory=True)
        print("x shape:", x.shape)
        print("y shape:", y.shape)
        ...
        x, y = seob_loader(21, 20, 0.01, 0.03, trajectory=True)
        print("x shape:", x.shape)
        print("y shape:", y.shape)
        ...
        # test trajectories
        x, y = seob_loader(10, 10, 0.01, 0.03, trajectory=True)
        print("x shape:", x.shape)
        print("y shape:", y.shape)
        # save the training data
