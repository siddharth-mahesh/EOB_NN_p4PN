...
class Neural_EOB(eqx.Module):
    ...
    def _pade_1_4(self,a_1, a_3, a_4, a_5, x):
        """
    ...
    def _pade_1_3(self,x,d_2, d_3, d_4):
        """
        Compute the Pade approximant P^1_3 for the Hamiltonian D potential.
        ...
        """
        tmp0 = (1.0/(d_3))
        tmp1 = tmp0*x*(((d_2)*(d_2)) - d_4) + 1
        pade_1_3 = tmp1/(-d_2*((x)*(x)) + tmp0*((x)*(x)*(x))*(-((d_2)*(d_2)*(d_2)) + d_2*d_4 - ((d_3)*(d_3))) + tmp1)
        return pade_1_3
    ...
    def _a_potential(self, r, nu, constants):
        ...
        a = self._pade_1_4(u, constants["a_1"], constants["a_3"], constants["a_4"], self.A_p4PN(neural_in))
        return a
    ...
    def _d_potential(self, r, nu, constants):
        ...
        u = 1 / r
        neural_in = jnp.array([u,nu])
        d = self._pade_1_3(u, constants["d_2"], constants["d_3"], self.D_p4PN(neural_in))
        return d
    ...
    def _hamiltonian(self, y, nu, constants):
        ...
        h_real = jnp.sqrt(2*nu*(jnp.sqrt(a*(((p_phi)*(p_phi))*((u)*(u)) + ((p_r)*(p_r))*(a/d + ((p_r)*(p_r))*(((u)*(u))*z_3 + q_p4pn)) + 1)) - 1) + 1)/nu
        return h_real
