...
class DHNN_Model(eqx.Module):
    ...
    def __init__(self, key, hidden_dim=256, input_dim = 3):
        """
    ...
    def _single_forward(self, z):
        ...
        dz = (
            symplectic_map @ (dH[:num_canonical_variables])
            + dD[:num_canonical_variables]
        )
        return dz
    ...
    def __call__(self, z, rhs=True):
        ...
        if rhs:
            ...
            return jnp.stack(
                [
                    jax.vmap(self._get_h, in_axes=0)(z),
                    jax.vmap(self._get_d, in_axes=0)(z),
                ],
                axis=-1,
            )

def generate_model_instance(seed = 5678, hidden_dim=256, input_dim = 3):
    key = jax.random.PRNGKey(seed)
    return DHNN_Model(key, hidden_dim, input_dim)

if __name__ == "__main__":
    dhnn = generate_model_instance()
    z0 = jax.random.uniform(jax.random.PRNGKey(42), (100, 3), minval=-1, maxval=1)
    rhs = dhnn(z0)
    print(z0.shape,rhs.shape)
