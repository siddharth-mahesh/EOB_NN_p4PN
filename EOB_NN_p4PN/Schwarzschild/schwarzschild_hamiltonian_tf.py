...
"""
    Code to calibrate geodesic Hamiltonians with gravitational wave data.
    The module generates training waveforms by integrating geodesics in Schwarzschild geometry
    and computing the instantaneous strain.
    The module then trains a Hamiltonian Neural Network (HNN) to learn the Hamiltonian.
"""
# Core libraries
import numpy as np
...
def plot_potential(model, grid_size=100, r_range=(7, 200),train_range=(7, 35)):
    ...
    np.savetxt('train_range.dat',np.array([train_range[0],train_range[1]]))
    np.savetxt('r_vals.dat',np.array(r_vals.numpy()))
    np.savetxt('learned_potentials.dat',np.array(learned_potentials))
    np.savetxt('true_potentials.dat',np.array(true_potentials))
    # Plot the learned potential and true potential
    plt.figure(figsize=(8, 6))
    plt.plot(r_vals, learned_potentials, label='Learned A(r)', lw=2)
    plt.plot(r_vals, true_potentials, label='True A(r)', linestyle='--', color='red')
    plt.axvline(x=train_range[0], color='green', linestyle=':', label='Training Range')
    plt.axvline(x=train_range[1], color='green', linestyle=':')
